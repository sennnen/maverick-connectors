...
//! - **v0x0b** (1924 bytes): full-resolution `u32` optical channels; not decoded here (no
//!   derived-metric maths are done on-strap, so the raw channels are surfaced and left for the host).
//!
//! Channel identity was pinned with an electrode-contact control capture: the middle channel rails
//! between the ADC extremes when the electrode floats and settles to a stable biopotential on skin
//! contact — the signature of the ECG lead. The outer two are optical and unaffected by contact.
